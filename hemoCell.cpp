...
HemoCell::HemoCell(char * configFileName, int argc, char * argv[], HemoCell::MPIHandle mpi_handler)
{
  ...
  string caseName =  (*cfg)["UDV"]["caseName"].read<string>();

  pcout << "Case name is " << caseName << endl;
...
void HemoCell::latticeEquilibrium(T rho, hemo::Array<T, 3> vel) {
  hlog << "Setting Fluid Equilibrium" << endl;
...
void HemoCell::setSystemPeriodicity(unsigned int axis, bool bePeriodic) {
  if (lattice == 0) {
    pcerr << "Please create a lattice before trying to set the periodicity" << endl;
    exit(1);    
  }
  if (cellfields->immersedParticles == 0) {
    pcerr << "Please create a particlefield (hemocell.initializeCellfields()) before trying to set the periodicity" << endl;
    exit(1);   
  }
...
void HemoCell::setSystemPeriodicityLimit(unsigned int axis, int limit) {
  hlog << "Setting periodicity limit of axis " << axis << " to " << limit << endl;
...
void HemoCell::loadParticles() {
  hlog << "Loading particle positions "  << endl;
...
void HemoCell::saveCheckPoint() {
  hlog << "Saving Checkpoint at timestep " << iter << endl;
...
void HemoCell::writeOutput() {
  ...
  pcout << "Writing output at timestep " << iter << " (" << param::dt * iter<< " s). Approx. performance: " << tpi << " s / iteration." << endl;
  ...
  if (global.cellsDeletedInfo) {
    ...
    pcout << "Cell or Cells Deleted //(-_-')\\" << endl;
...
void HemoCell::setRepulsionTimeScaleSeperation(unsigned int separation){
  hlog << "Setting timescale seperation for repulsion to " << separation << " timesteps"<<endl;
...
void HemoCell::ActivateRepulsion(){
string RepulsionActive =  (*cfg)["domain"]["RepulsionActive"].read<string>();

if (RepulsionActive == "True")
{
  setRepulsion((*cfg)["domain"]["kRep"].read<T>(), (*cfg)["domain"]["RepCutoff"].read<T>());
  setRepulsionTimeScaleSeperation((*cfg)["domain"]["repulsionStep"].read<int>());
  enableBoundaryParticles((*cfg)["domain"]["kRep"].read<T>(), (*cfg)["domain"]["RepCutoff"].read<T>(),(*cfg)["domain"]["repulsionStep"].read<int>());
}
}

void HemoCell::setRepulsion(T repulsionConstant, T repulsionCutoff) {
  hlog << "Setting repulsion constant to " << repulsionConstant << ". repulsionCutoff to " << repulsionCutoff << " LU" << endl;
  hlogfile << "Enabling repulsion." << endl;
  cellfields->repulsionConstant = repulsionConstant;
  cellfields->repulsionCutoff = repulsionCutoff;
...
void HemoCell::enableBoundaryParticles(T boundaryRepulsionConstant, T boundaryRepulsionCutoff, unsigned int timestep) {
  ...
  hlog << "Setting boundary repulsion constant to " << boundaryRepulsionConstant << ". boundary repulsionCutoff to " << boundaryRepulsionCutoff << " LU" << endl;
  hlogfile << "Enabling boundary repulsion" << endl;
  cellfields->boundaryRepulsionConstant = boundaryRepulsionConstant;
  cellfields->boundaryRepulsionCutoff = boundaryRepulsionCutoff;
...
void HemoCell::sanityCheck() {
  hlog << "Performing Sanity check on simulation parameters and setup" << endl;
  //Lattice sanity
  ...
  //Parameter Sanity
#ifdef FORCE_LIMIT
  hlog << "Force limit active at " << FORCE_LIMIT << " pN." << endl;
#endif
