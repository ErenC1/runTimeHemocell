...
RbcHighOrderModel::RbcHighOrderModel(Config & modelCfg_, HemoCellField & cellField_) : CellMechanics(cellField_, modelCfg_),
                  ...
                  kScale(RbcHighOrderModel::calculate_kScale(modelCfg_)),
                  tauA(RbcHighOrderModel::calculate_tauA(modelCfg_)),
                  tauV(RbcHighOrderModel::calculate_tauV(modelCfg_)),
                  tauB(RbcHighOrderModel::calculate_tauB(modelCfg_)),
                  tauL(RbcHighOrderModel::calculate_tauL(modelCfg_)),
                  //k_ES( RbcHighOrderModel::calculate_kArea(modelCfg_,*cellField_.meshmetric) ), 
                  //k_AD( RbcHighOrderModel::calculate_kArea(modelCfg_,*cellField_.meshmetric) ), 
                  //tauS(RbcHighOrderModel::calculate_tauL(modelCfg_)),
                  ...
                  //k_Surf( RbcHighOrderModel::calculate_kSurf(modelCfg_,*cellField_.meshmetric) ),
...
void RbcHighOrderModel::ParticleMechanics(map<int,vector<HemoCellParticle *>> & particles_per_cell, const map<int,bool> & lpc, size_t ctype) {
  ...
  for (const auto & pair : lpc) { //For all cells with at least one lsp in the local domain.
    ...
    for (const hemo::Array<plint,3> & triangle : cellConstants.triangle_list) {
      ...
      //standart
      //const T afm = k_area * (areaRatio+areaRatio/std::fabs(MaxCellSurfaceAreaChange-areaRatio*areaRatio))*kScale;
      
      //ez access
      const T afm = k_area * (areaRatio+areaRatio/std::fabs(tauA-areaRatio*areaRatio))*kScale;
 
      //ez access Skalak
      //const T afm = k_area * pow((area-cellConstants.triangle_area_eq_list[triangle_n]),2)/cellConstants.triangle_area_eq_list[triangle_n];
    ...
    //standart
    //const T volume_force = -k_volume * volume_frac/std::fabs(MaxCellVolumetricChange-volume_frac*volume_frac)*kScale;
    
    //ez access
    const T volume_force = -k_volume * volume_frac/std::fabs(tauV-volume_frac*volume_frac);

    //ez access Skalak
    //const T volume_force = -k_volume* 0.5 * pow((volume_frac-cellConstants.volume_eq),2)/cellConstants.volume_eq;
...
//Volume force loop
    ...
    for (long unsigned int i = 0 ; i < cell.size() ; i++) {
      ...
      //standart
      //const hemo::Array<T,3> bending_force = k_bend * ( dDev + dDev/std::fabs(MaxCellBendingAngle-dDev*dDev)) * patch_normal*kScale;
      
      //ez access
      const hemo::Array<T,3> bending_force = k_bend * ( dDev + dDev/std::fabs(tauB-dDev*dDev)) * patch_normal*kScale;

      //ez access Skalak
      //const T k_bendTilda = pow(3,0.5)*k_bend;
      //const hemo::Array<T,3> bending_force = k_bendTilda/2 * ( dDev * dDev) * patch_normal;
    ...
    for (const hemo::Array<plint,2> & edge : cellConstants.edge_list) {
      ...
      //standart
      //const T edge_force_scalar = k_link * ( edge_frac + edge_frac/std::fabs(MaxCellPersistenceLength-edge_frac*edge_frac))*kScale;
      
      //ez access
      const T edge_force_scalar = k_link * ( edge_frac + edge_frac/std::fabs(tauL-edge_frac*edge_frac))*kScale;

      //ez access - Skalak
      //const T edge_force_scalar = k_ES /12 * (lambda[0]*lambda[0]+2*lambda[0]-2*lambda[1])+k_AD/12*lambda[1]*lambda[1]
...
void RbcHighOrderModel::statistics() {
    ...
    hlog << "\t FORCES ARE SCALED BY:   " << kScale << std::endl; 
    hlog << "\t k_link:   " << k_link << "\t tauL:   " << tauL << std::endl; 
    hlog << "\t k_area:   " << k_area << "\t tauA:   " << tauA << std::endl; 
    hlog << "\t k_bend:   " << k_bend << "\t tauB:   " << tauB << std::endl; 
    hlog << "\t k_volume: " << k_volume << "\t tauV:   " << tauV << std::endl; 
